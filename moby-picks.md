# 莫比精选 · 批次 1「主力工作台」(2026-08-24 莫比亲手挑)

> 纪律:精选=莫比义务(判断层品味),不外包;每批亲手挑+亲手写理由(白箱:焦虑→解决→出处)。技术落地(四道门/bundle 落库)交屿。

## 套装「主力工作台」(5 件,把 DSH 当生产工具)

| # | 插件 | 焦虑 | 解决 | 出处 |
|---|------|------|------|------|
| 1 | dsh-memory | 换会话/上下文压缩就忘 | SQLite 持久化,写操作过审批门,快照自动注入 | PerryLi,MIT |
| 2 | dsh-usage-dashboard | 不知道钱烧哪了 | 余额撑几天/今日本月/最贵模型一屏看清 | Cassius0924,MIT |
| 3 | dsh-context-doctor | 上下文越塞越贵 | AGENTS.md/技能/schema token 成本体检+检测重复 | 公开仓库,MIT |
| 4 | dsh-subagent-tools | 子代理没谱一把抓 | 逐调用覆盖 model/provider/persona/toolFilter | MIT |
| 5 | dsh-proof | 验收靠自觉 | turn 末自动 spawn 只读验证代理挑缺口 | EvilIrving,MIT |

## 备选池(下批候选)
- dsh-trace-compare(执行轨迹可视化=Agent 可观测性,T51 相关)
- dsh-plugin-guard(安装安全网:快照/自动回退)
- dsh-deep-research(Deep Research 编排)
- dsh-science-workbench(可复现科研工作台)
- telegram(Telegram 桥接)

## 下一批触发
- 莫比每批亲手挑(每周≥1 批);上批理由入册,老套装按四层标准复核时同步更新。
